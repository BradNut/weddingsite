...
const Login = () => {
  ...
  async function handleSubmit({ username, password, penguin }: { username: string; password: string; penguin: string }) {
    const body = {
      username,
      password,
      penguin,
    ...
    <div>
      ...
      <FormStyles
        onSubmit={async (e) => {
          e.preventDefault();
          setLoading(true);
          await handleSubmit(inputs as { username: string; password: string; penguin: string });
